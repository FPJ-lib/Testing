...
#Tickers
list_tickers = ['^GSPC','GOVT']
name_columns = ['Stocks', 'Bond']
...
if get_data==1:
    ...
    df.to_csv('/Users/filipepessoajorge/OneDrive/X002 - Python_Developing/GitHub/Economic_data/Economic_data/AAA_Tese/Data_/portfolio.csv')
else:
    df = pd.read_csv('/Users/filipepessoajorge/OneDrive/X002 - Python_Developing/GitHub/Economic_data/Economic_data/AAA_Tese/Data_/portfolio.csv')
...
'''Plot Volatilities'''
vol = pd.DataFrame()
vol['Stocks'] = df['Stocks'].rolling(window=50).std() * 100
vol['Bond'] = df['Bond'].rolling(window=50).std() * 100
